...
public class MinioApplication {
    ...
    /**
     * Переименовывает файл в MinIO.
     * @param serverFilePath Текущий путь к файлу.
     * @param newFileName Новое имя файла (без пути).
     */
    public void fileNameChange(String serverFilePath, String newFileName) {
        // Проверка входных данных
        if (serverFilePath == null || serverFilePath.isEmpty() || newFileName == null || newFileName.isEmpty()) {
            Logger.error("Ошибка: Не указан текущий путь или новое имя файла.");
            return;
        }
        if (newFileName.contains("/")) {
            Logger.error("Ошибка: Новое имя файла не должно содержать '/'. Указано: {}", newFileName);
            return;
        }

        // Определяем директорию и старое имя файла
        String directoryPath = "";
        String oldFileName;
        int lastSlashIndex = serverFilePath.lastIndexOf('/');
        if (lastSlashIndex != -1) {
            directoryPath = serverFilePath.substring(0, lastSlashIndex + 1);
            oldFileName = serverFilePath.substring(lastSlashIndex + 1);
        } else {
            oldFileName = serverFilePath;
        }

        // Если новое имя совпадает со старым, ничего не делаем
        if (oldFileName.equals(newFileName)) {
            Logger.warn("Новое имя файла совпадает со старым. Переименование не требуется: {}", serverFilePath);
            return;
        }

        String newServerFilePath = directoryPath + newFileName;
        ...
        try {
            ...
            try {
                ...
                Logger.info("Исходный файл {} найден.", serverFilePath);
            } catch (ErrorResponseException e) {
                if (e.errorResponse() != null && "NoSuchKey".equals(e.errorResponse().code())) {
                    Logger.error("Ошибка переименования: Исходный файл {} не найден.", serverFilePath);
                } else {
                    Logger.error("Ошибка при проверке существования исходного файла {}: {}", serverFilePath, e.getMessage());
                }
                return; // Прерываем, если исходный файл не найден
            }

            // 2. Проверяем, не существует ли уже файл с новым именем
            try {
                minioClient.statObject(
                    StatObjectArgs.builder()
                        .bucket(bucketName)
                        .object(newServerFilePath)
                        .build()
                );
                // Если statObject не выбросил исключение, значит файл существует
                Logger.error("Ошибка переименования: Файл с именем {} уже существует в директории {}.", newFileName, directoryPath.isEmpty() ? "корневой" : directoryPath);
                return;
            } catch (ErrorResponseException e) {
                // Ожидаем ошибку "NoSuchKey", если файла с новым именем нет
                if (e.errorResponse() == null || !"NoSuchKey".equals(e.errorResponse().code())) {
                    // Если ошибка другая, логируем ее и прерываем
                    Logger.error("Ошибка при проверке существования целевого файла {}: {}", newServerFilePath, e.getMessage());
                    return;
                }
                // Если "NoSuchKey", то всё хорошо, можно продолжать
                Logger.info("Файл с именем {} не существует. Можно переименовывать.", newServerFilePath);
            }


            // 3. Копируем объект с новым именем
            Logger.info("Начало копирования {} в {}", serverFilePath, newServerFilePath);
            minioClient.copyObject(
                CopyObjectArgs.builder()
                    .source(CopySource.builder()
                        .bucket(bucketName)
            ...
            Logger.info("Файл {} успешно скопирован в {}.", serverFilePath, newServerFilePath);

            // 4. Удаляем исходный объект после успешного копирования
            Logger.info("Начало удаления исходного файла {}", serverFilePath);
            minioClient.removeObject(
                RemoveObjectArgs.builder()
                    .bucket(bucketName)
                    .object(serverFilePath)
            ...
            Logger.info("Исходный файл {} успешно удален.", serverFilePath);

            System.out.println("Файл \"" + serverFilePath + "\" успешно переименован в \"" + newServerFilePath + "\"");

        } catch (ErrorResponseException e) {
            Logger.error("Ошибка ответа MinIO при переименовании файла {} в {}: Код={}, Сообщение={}", serverFilePath, newServerFilePath, e.errorResponse() != null ? e.errorResponse().code() : "N/A", e.getMessage());
            e.printStackTrace();
        } catch (InsufficientDataException | InternalException | InvalidResponseException | IOException | ServerException | XmlParserException | InvalidKeyException | NoSuchAlgorithmException e) {
            Logger.error("Ошибка MinIO/сети/безопасности при переименовании файла {} в {}: {}", serverFilePath, newServerFilePath, e.getMessage());
            e.printStackTrace();
        } catch (Exception e) {
            Logger.error("Непредвиденная ошибка при переименовании файла {} в {}: {}", serverFilePath, newServerFilePath, e.getMessage());
            e.printStackTrace();
        }
    }
